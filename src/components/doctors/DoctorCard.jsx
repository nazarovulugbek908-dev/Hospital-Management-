...
export function DoctorCard({ doctor, onBook }) {
  ...
  return (
    <div className="group rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-5 sm:p-6 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between space-y-4">
      ...
      {/* Action Buttons */}
      <div className="flex flex-col xs:flex-row sm:flex-row items-stretch sm:items-center gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
        <Link
          to={`/patient/doctors/${doctor.id}`}
          className="flex-1 py-2.5 px-3 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 text-xs font-bold text-center transition-colors flex items-center justify-center"
        >
          {t('viewProfile')}
        </Link>
        ...
        {onBook ? (
          ...
          <Link
            ...
            className="flex-1 py-2.5 px-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold text-center shadow-md shadow-blue-600/25 transition-all flex items-center justify-center gap-1"
          >
